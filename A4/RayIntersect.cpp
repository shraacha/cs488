    ...
    for (size_t i = 0; i < 6; ++i) {
        ...
        case 0: // back face
            result = findRayPolygonIntersectAndNormal(
                eye, pixel,
                {glm::dvec4(corner.x, corner.y, corner.z, 1.0),
        ...
        case 3: // front face
            result = findRayPolygonIntersectAndNormal(
                eye, pixel,
                {glm::dvec4(corner.x + width, corner.y + width,
                            corner.z + width, 1.0),
                 glm::dvec4(corner.x, corner.y + width, corner.z + width, 1.0),
                 glm::dvec4(corner.x, corner.y, corner.z + width, 1.0),
                 glm::dvec4(corner.x + width, corner.y, corner.z + width,
                            1.0)});
            break;
        case 4: // right face
            result = findRayPolygonIntersectAndNormal(
                ...
                {glm::dvec4(corner.x + width, corner.y + width,
                 ...
                 glm::dvec4(corner.x + width, corner.y, corner.z + width, 1.0),
                 glm::dvec4(corner.x + width, corner.y, corner.z, 1.0),
                 glm::dvec4(corner.x + width, corner.y + width, corner.z,
                            1.0)});
            break;
        default: // top face
            result = findRayPolygonIntersectAndNormal(
                ...
                {glm::dvec4(corner.x + width, corner.y + width,
                 ...
                 glm::dvec4(corner.x + width, corner.y + width, corner.z, 1.0),
                 glm::dvec4(corner.x, corner.y + width, corner.z, 1.0),
                 glm::dvec4(corner.x, corner.y + width, corner.z + width,
                            1.0)});
            break;
        }
...
// -------------------------------------------------
std::optional<std::pair<double, glm::dvec4>>
findRayPolygonIntersectAndNormal(const glm::dvec4 &eye,
                                 ...
                                 const std::vector<glm::dvec4> &vertices)
{
    ...
    if (t.has_value())
    {
        ...
        for(size_t i = 0; i < vertices.size(); ++i)
        {
            ...
            glm::dvec2 v2 = projectOntoPlane(vertices[(i + 1) % vertices.size()], closestCoordinatePlane);

            if (!isInBoundsClip(projectedPoint, v1, {-v2.y + v1.y, v2.x - v1.x}))
            {
                return std::nullopt;
            }
        }
