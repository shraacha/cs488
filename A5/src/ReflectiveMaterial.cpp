...
glm::dvec3 ReflectiveMaterial::getReflectedRadiance(
    ...
    const glm::dvec3 & reflectionRadiance) const
{
    ...
    double nDotL =
        std::max(glm::dot(surfaceNormal, normalizedLightVector), 0.0);
    lightOut +=
        scaleFactor * (kD * getAlbedo() / M_PI + specular) * radiance * nDotL;

    return lightOut;
}

glm::dvec3 ReflectiveMaterial::getRefractedRadiance(
    const Ray & ray, const Intersection & intersect,
