...
int gr_render_cmd(lua_State* L)
{
  ...
  lua_isboolean(L, 12);
  bool visualizePhotons = lua_toboolean(L, 12);

  double photonRadiusVisualization = luaL_checknumber(L, 13);

  unsigned int numPhotons = luaL_checknumber(L, 14);
...
extern "C"
int gr_material_reflective_cmd(lua_State *L)
{
    GRLUA_DEBUG_CALL;
    ...
    data->material =
        new ReflectiveMaterial(glm::vec3(albedo[0], albedo[1], albedo[2]), roughness);
